...
"""Receive-side protocol mixin for MooncakeConnectorWorker."""
import asyncio
import logging
...
class _WorkerReceiveMixin:
    """All receive-side protocol logic for MooncakeConnectorWorker."""
    ...
    async def handle_new_engine_id(
        ...
        remote_bootstrap_addr = next(
            iter(pull_metas.values())
    ...
    async def receive_kv_from_single_worker(
        ...
        try:
            ...
            logger.error(
                "MooncakeXferMetadata transfer failed for %s: %s", req_ids, e
            )
            self.xfer_stats.record_failed_recv()
            return
