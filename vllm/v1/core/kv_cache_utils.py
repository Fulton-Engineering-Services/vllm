...
def get_kv_cache_groups(
    ...
    # GLM-5.3-Flash case (MLA target + kpool indexer at their REAL page sizes
    # + mamba groups slot-sharing the MLA tensors + kpool-tail and DFlash2
    # drafter groups): keeps the indexer at its real 8.4 KiB page instead of
    # padding it to the 1.125 MiB MLA page (which collapses the pool ~8x).
    # Must precede group_and_unify_kv_cache_specs, which would unify the
    # indexer onto the MLA page, and the generic uniform-page path, which
    # cannot serve this model. Returns None for non-GLM-5.3 spec sets.
    from vllm.v1.glm5next.kv_groups import try_build_kv_cache_groups

    if glm5_groups := try_build_kv_cache_groups(vllm_config, kv_cache_spec):
        return glm5_groups
