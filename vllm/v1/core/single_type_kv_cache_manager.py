...
class KpoolTailManager(SlidingWindowManager):
    ...
    def _live_span(self, num_tokens: int, num_tokens_main_model: int) -> tuple[int, int]:
    ...
    def remove_skipped_blocks(
        ...
        if not blocks:
            return
        skipped = self.get_num_skipped_tokens(processed_computed_tokens) // self.block_size
