...
{
    NixlAgentInfo(std::shared_ptr<nixlAgent> agent,
                  nixlBackendH* backend,
                  int max_num_ranks,
                  nixl_mem_t mem_type)
        : agent(agent),
          backend(backend),
          rdma_reg_descs(mem_type),
          sync_reg_descs(mem_type),
          sync_count_reg_descs(mem_type),
          ht_barrier_reg_descs(mem_type) {
...
private:
    ...
    /* Owning pinned-host allocations used in host-staging mode */
    std::unique_ptr<pinned_host_region> m_rdma_host_alloc;
    std::unique_ptr<pinned_host_region> m_mask_host_alloc;
    std::unique_ptr<pinned_host_region> m_sync_host_alloc;
    std::unique_ptr<pinned_host_region> m_sync_count_host_alloc;
    std::unique_ptr<pinned_host_region> m_ht_barrier_host_alloc;
...
public:
    Buffer(int rank, bool explicitly_destroy, bool low_latency_mode, int timeout_ms, bool host_staging = false);
