...
constexpr const char *k_host_ctx = "pinned_host_region";
...
void
warn_cuda(cudaError_t status, const char *context, const char *operation) noexcept {
...
void
pinned_host_region::release() noexcept {
    if (ptr_ != nullptr) {
        warn_cuda(cudaFreeHost(ptr_), k_host_ctx, "cudaFreeHost");
        ptr_ = nullptr;
    }
}

pinned_host_region::~pinned_host_region() {
    release();
}

pinned_host_region::pinned_host_region(size_t size) {
    if (size == 0) {
        throw std::invalid_argument("pinned_host_region: size must be non-zero");
    }
    void *ptr = nullptr;
