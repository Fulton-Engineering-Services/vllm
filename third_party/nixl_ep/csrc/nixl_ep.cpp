...
Buffer::Buffer(int rank, bool explicitly_destroy, bool low_latency_mode, int timeout_ms, bool host_staging):
...
void Buffer::init(int num_ranks, int num_experts_per_rank, int64_t num_nvl_bytes, int64_t num_rdma_bytes)
{
    ...
    EP_HOST_ASSERT(not host_staging or num_nvl_bytes == 0);

    if (host_staging) {
        m_rdma_host_alloc = std::make_unique<pinned_host_region>(static_cast<size_t>(num_rdma_bytes));
        rdma_buffer_ptr = m_rdma_host_alloc->ptr();
        std::memset(rdma_buffer_ptr, 0, num_rdma_bytes);
    } else {
    ...
    if (host_staging) {
        m_mask_host_alloc = std::make_unique<pinned_host_region>(static_cast<size_t>(num_mask_buffer_bytes));
        mask_buffer_ptr = static_cast<int *>(m_mask_host_alloc->ptr());
        std::memset(mask_buffer_ptr, 0xff, num_mask_buffer_bytes);
        std::memset(mask_buffer_ptr + rank, 0, sizeof(int));
    } else {
    ...
    if (host_staging) {
        m_sync_host_alloc = std::make_unique<pinned_host_region>(static_cast<size_t>(num_sync_buffer_bytes));
        sync_buffer_ptr = static_cast<int *>(m_sync_host_alloc->ptr());
        m_sync_count_host_alloc = std::make_unique<pinned_host_region>(static_cast<size_t>(num_sync_buffer_bytes));
        sync_count_ptr = static_cast<int *>(m_sync_count_host_alloc->ptr());
        std::memset(sync_buffer_ptr, 0, num_sync_buffer_bytes);
        std::memset(sync_count_ptr, 0, num_sync_buffer_bytes);
    } else {
    ...
    if (!low_latency_mode) {
        if (host_staging) {
            m_ht_barrier_host_alloc = std::make_unique<pinned_host_region>(sizeof(uint64_t));
            local_ht_barrier_counter = static_cast<uint64_t *>(m_ht_barrier_host_alloc->ptr());
            std::memset(local_ht_barrier_counter, 0, sizeof(uint64_t));
        } else {
...
torch::Tensor Buffer::get_local_buffer_tensor(const pybind11::object& dtype, int64_t offset, bool use_rdma_buffer) const {
    ...
    // In host-staging mode the RDMA buffer lives in pinned host memory, so
    // return a CPU tensor. The underlying memory is still accessible by the GPU
    // via the host page tables (zero-copy on unified-memory platforms).
    if (use_rdma_buffer and host_staging) {
        return torch::from_blob(base_ptr, num_bytes / element_bytes, torch::TensorOptions().dtype(casted_dtype).device(at::kCPU));
    }
...
void Buffer::destroy() {
    ...
    m_rdma_alloc.reset();
    m_rdma_host_alloc.reset();
    rdma_buffer_ptr = nullptr;
    m_mask_alloc.reset();
    m_mask_host_alloc.reset();
    mask_buffer_ptr = nullptr;
    m_sync_alloc.reset();
    m_sync_host_alloc.reset();
    sync_buffer_ptr = nullptr;
    m_sync_count_alloc.reset();
    m_sync_count_host_alloc.reset();
    sync_count_ptr = nullptr;

    if (!low_latency_mode) {
        if (host_staging) {
            m_ht_barrier_host_alloc.reset();
        } else {
...
void Buffer::_nixl_agents_connect(const std::vector<int>& ranks, const std::vector<nixl_blob_t>& remote_mds) {
    ...
    while (peers_remaining > 0) {
        for (int remote_rank : ranks) {
            ...
            nixl_xfer_dlist_t empty_descs(host_staging ? DRAM_SEG : VRAM_SEG);
    ...
    for (int remote_rank : ranks) {
        do {
            ...
            for (auto &notif : notif_map) {
                ...
                if (remote_peer_info.host_staging != host_staging) {
                    throw std::runtime_error(
                        "NIXL EP mixed staging mode detected for remote rank " +
                        std::to_string(remote_peer_info.rank) +
                        ": remote host_staging=" +
                        (remote_peer_info.host_staging ? std::string("true") : std::string("false")) +
                        ", local host_staging=" +
                        (host_staging ? std::string("true") : std::string("false")) +
                        ". All ranks must use the same VLLM_NIXL_EP_HOST_STAGING setting.");
                }
...
void Buffer::_nixl_ep_memory_views_create(void) {
    auto mem_type = host_staging ? DRAM_SEG : VRAM_SEG;
    nixl_remote_dlist_t remote_descs(mem_type);
    nixl_remote_dlist_t barrier_descs(mem_type);
    nixl_local_dlist_t local_descs(mem_type);

    uint64_t local_desc_device_id = host_staging ? 0 : get_local_device_id();
    local_descs.addDesc(nixlBlobDesc((uintptr_t)(rdma_buffer_ptr), num_rdma_bytes, local_desc_device_id, ""));
    local_descs.addDesc(nixlBlobDesc((uintptr_t)(sync_count_ptr), max_num_ranks * sizeof(int), local_desc_device_id, ""));
    ...
    if (!remote_ranks.empty()) {
        ...
        if (!low_latency_mode && max_num_ranks > NUM_MAX_NVL_PEERS) {
            nixl_remote_dlist_t ht_barrier_descs(mem_type);
            for (int r = 0; r < max_num_ranks; r++) {
                std::string remote_agent_name = remote_set.count(r) ? nixl_agent_info->remote_agent_names[r] : nixl_null_agent;
...
void Buffer::_nixl_agent_init() {
    ...
    uint64_t desc_device_id = host_staging ? 0 : device_id;
    nixl_agent_info = std::make_unique<NixlAgentInfo>(
        agent, ucx_backend, max_num_ranks,
        host_staging ? DRAM_SEG : VRAM_SEG);
    ...
    nixl_agent_info->rdma_reg_descs.addDesc(
        nixlBlobDesc(reinterpret_cast<uintptr_t>(rdma_buffer_ptr), num_rdma_bytes, desc_device_id, ""));

    nixl_agent_info->sync_reg_descs.clear();
    nixl_agent_info->sync_reg_descs.addDesc(
        nixlBlobDesc(reinterpret_cast<uintptr_t>(sync_buffer_ptr), max_num_ranks * sizeof(int), desc_device_id, ""));

    nixl_agent_info->sync_count_reg_descs.clear();
    nixl_agent_info->sync_count_reg_descs.addDesc(
        nixlBlobDesc(reinterpret_cast<uintptr_t>(sync_count_ptr), max_num_ranks * sizeof(int), desc_device_id, ""));
    ...
    if (local_ht_barrier_counter) {
        nixl_agent_info->ht_barrier_reg_descs.addDesc(
            nixlBlobDesc((uintptr_t)(local_ht_barrier_counter), sizeof(uint64_t), desc_device_id, ""));
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    pybind11::class_<nixl_ep::Buffer>(m, "Buffer")
        .def(pybind11::init<int, bool, bool, int, bool>(),
             py::arg("rank"), py::arg("explicitly_destroy"),
             py::arg("low_latency_mode"), py::arg("timeout_ms"),
             py::arg("host_staging") = false)
